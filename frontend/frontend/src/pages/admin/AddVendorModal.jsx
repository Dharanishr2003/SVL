...
const INITIAL_FORM = {
  ...
  password: "",
  vendorTypeIds: [],
  countryOfRegistration: DEFAULT_COUNTRY_OF_REGISTRATION,
...
export default function AddVendorModal({ open, onClose, onCreated }) {
  ...
  useEffect(() => {
    ...
    Promise.all([getVendorTypes(), getServiceCategories(), getServiceTypes()])
      .then(([types, cats, serviceTypes]) => {
        ...
        setAllTypes(Array.isArray(serviceTypes) ? serviceTypes : []);
      })
      .catch(() => {});
  }, [open]);
  ...
  const typeOptions = useMemo(() => {
    ...
    return allTypes.filter(
      (t) => String(t.categoryId) === String(pickCategoryId) && !t.parentId,
    );
  }, [allTypes, pickCategoryId]);

  const subtypeOptions = useMemo(() => {
    if (!pickTypeId) return [];
    return allTypes.filter((t) => String(t.parentId) === String(pickTypeId));
  }, [allTypes, pickTypeId]);

  const hasSubtypes = subtypeOptions.length > 0;

  const handleCategoryChange = (val) => {
    setPickCategoryId(val);
    setPickTypeId("");
    setPickSubtypeId("");
  };
  ...
  const handleAddService = () => {
    ...
    const cat = categories.find((c) => String(c.id) === String(pickCategoryId));
    const type = allTypes.find((t) => String(t.id) === String(pickTypeId));
    const subtype = pickSubtypeId
      ? allTypes.find((t) => String(t.id) === String(pickSubtypeId))
      : null;

    const label = [cat?.name, type?.name, subtype?.name].filter(Boolean).join(" > ");
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const message = extractApiErrorMessage(e, "Failed to add vendor");
    ...
    <div className="avm-backdrop">
      <div className="avm-modal" onClick={(e) => e.stopPropagation()}>
        <div className="avm-modal-header">
          <h2 className="avm-modal-title">New Vendor</h2>
          ...
          {step === 0 && (
            <>
              ...
              <div className="avm-grid">
                ...
                <div className="avm-field">
                  <label className="avm-label">Password</label>
