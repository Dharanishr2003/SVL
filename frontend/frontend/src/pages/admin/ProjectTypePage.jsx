...
function ProjectTypePage() {
  ...
  const loadProjectType = async () => {
    ...
    try {
      ...
      showError(extractApiErrorMessage(err, "Failed to load project type list"));
      setRows([]);
    } finally {
      setLoading(false);
    }
  ...
  const filteredRows = useMemo(() => {
    ...
    return orderedRows.filter((r) => {
      const type = String(r.projectType || "").toLowerCase();
      const typeId = String(r.typeId || "").toLowerCase();
      return type.includes(term) || typeId.includes(term);
    });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      showError(extractApiErrorMessage(err, "Failed to save project type"));
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      showError(extractApiErrorMessage(err, "Failed to delete project type"));
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="container-fluid content">
      <div className="card border-0 shadow-sm p-4 mb-4 bg-white" style={{ borderRadius: 12 }}>
        <div className="d-flex align-items-center justify-content-between flex-wrap gap-3">
          <div>
            <h2 className="leads-header-title mb-1" style={{ fontSize: "1.4rem", fontWeight: "700", color: "#0f172a" }}>Project Type</h2>
            <p className="leads-header-subtitle text-muted mb-0" style={{ fontSize: "0.9rem" }}>
              Configure and manage lead classification styles for project assignments.
            </p>
          </div>
            ...
            <table className="table table-hover align-middle leads-table mb-0">
              <thead>
                <tr>
                  ...
                  <th className="text-muted" style={{ fontWeight: "600", fontSize: "0.85rem" }}>Project Type</th>
              ...
              <tbody>
                {loading ? (
                  <tr>
                    <td colSpan={canManageProjectType ? 4 : 3} className="text-center py-4 text-muted">Loading...</td>
                  </tr>
                ) : pagedRows.length === 0 ? (
                  <tr>
                    <td colSpan={canManageProjectType ? 4 : 3} className="text-center py-4 text-muted">No records found</td>
                  </tr>
                ) : (
                  pagedRows.map((row, i) => (
                    <tr key={row.id}>
                      <td className="fw-semibold" style={{ color: "#1e293b", fontSize: "0.9rem" }}>{(page - 1) * pageSize + i + 1}</td>
                      <td className="fw-semibold" style={{ color: "#0f172a", fontSize: "0.9rem" }}>{row.projectType || "-"}</td>
      ...
      {showCreateModal && canManageProjectType && (
        <>
          <div
              ...
              <div className="modal-content border-0 shadow-lg" style={{ borderRadius: 14 }}>
                <div className="modal-header bg-light">
                  <h5 className="modal-title fw-bold" style={{ color: "#334155" }}>
                    {editingRow ? "Edit Project Type" : "Create Project Type"}
                  </h5>
                  <button
                    className="btn-close"
      ...
      {pendingDeleteRow && canManageProjectType && (
        <>
          <div
            className="modal fade show"
            style={{ display: "block", zIndex: 1060 }}
          ...
          >
            <div className="modal-dialog modal-dialog-centered modal-sm">
              <div className="modal-content border-0 shadow-lg" style={{ borderRadius: 14 }}>
                <div className="modal-header bg-light">
                  <h5 className="modal-title fw-bold" style={{ color: "#334155" }}>Delete Project Type</h5>
                  <button
                    className="btn-close"
                ...
                <div className="modal-body p-4 text-center">
                  ...
                  <p className="mb-0">
                    Are you sure you want to delete <strong>{pendingDeleteRow?.projectType || ""}</strong>?
                  </p>
                </div>
