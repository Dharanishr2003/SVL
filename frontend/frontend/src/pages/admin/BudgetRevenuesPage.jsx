...
import { getBudgetRevenues, createBudgetRevenue, updateBudgetRevenue, deleteBudgetRevenue } from "../../api/budgetRevenuesApi";
...
export default function BudgetRevenuesPage() {
  ...
  const loadRevenues = async () => {
    setLoading(true);
    try {
      const data = await getBudgetRevenues();
      setRows(Array.isArray(data) ? data : []);
    } catch (e) {
      showError(extractApiErrorMessage(e, "Failed to load revenues"));
    } finally {
      setLoading(false);
    }
  };
  ...
  const exportExcel = () => {
    const headers = ["Revenue Name", "Category Name", "Sub Category Name", "Amount", "Date"];
    ...
    const a = document.createElement("a"); a.href = url; a.download = `budget-revenues-${Date.now()}.xls`; a.click();
  };
  ...
  const exportCsv = () => {
    ...
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a"); a.href = url; a.download = `budget-revenues-${Date.now()}.csv`; a.click();
  };

  const exportPdf = () => {
    const doc = new jsPDF({ orientation: "portrait", unit: "pt", format: "a4" });
    doc.setFontSize(14); doc.text("Budget Revenues Report", 40, 40);
    doc.setFontSize(10); doc.text(`Generated: ${new Date().toLocaleString()}`, 40, 58);
    autoTable(doc, {
      head: [["Revenue Name", "Category Name", "Sub Category Name", "Amount", "Date"]],
    ...
    doc.save(`budget-revenues-${Date.now()}.pdf`);
  };
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      await createBudgetRevenue(payload);
      showSuccess("Revenue added successfully");
      setForm(initialForm);
      setShowAddModal(false);
      await loadRevenues();
    } catch (err) {
      showError(extractApiErrorMessage(err, "Failed to add revenue"));
    }
  };
  ...
  const handleEdit = async (e) => {
    ...
    try {
      ...
      await updateBudgetRevenue(editTarget.id, payload);
      showSuccess("Revenue updated successfully");
      setShowEditModal(false);
      await loadRevenues();
    } catch (err) {
      showError(extractApiErrorMessage(err, "Failed to update revenue"));
    }
  };

  const handleDelete = async () => {
    try {
      await deleteBudgetRevenue(deleteTarget.id);
      ...
      showError(extractApiErrorMessage(err, "Failed to delete revenue"));
    }
  };
      ...
      <div className="content">
        ...
        <div className="card border-0 shadow-sm mb-4 bg-white" style={{ borderRadius: 12 }}>
          <div className="card-body p-4 d-flex justify-content-between align-items-center flex-wrap gap-3">
            <div>
              <h3 className="fw-bold mb-1 text-slate-800" style={{ fontSize: "1.3rem" }}>Budget Revenues</h3>
              <nav aria-label="breadcrumb">
                <ol className="breadcrumb mb-0" style={{ fontSize: "0.85rem" }}>
                  ...
                  <li className="breadcrumb-item active text-primary" aria-current="page">Budget Revenues</li>
                </ol>
              </nav>
            </div>
            <button
            ...
            >
              <i className="ti ti-plus" /> Add Revenue
            </button>
          </div>
        </div>
        ...
        <div className="card border-0 shadow-sm bg-white" style={{ borderRadius: 12, overflow: "hidden" }}>
          ...
          <div className="p-3 border-bottom d-flex flex-wrap align-items-center justify-content-between gap-3 bg-light">
            ...
            <div className="dropdown">
              <button
                ...
                type="button"
                id="budgetRevenuesExportDropdown"
              ...
              <ul className="dropdown-menu shadow border-0" aria-labelledby="budgetRevenuesExportDropdown">
          ...
          <div className="table-responsive">
            <table className="table table-hover align-middle mb-0">
              ...
              <tbody>
                {pagedRows.length === 0 ? (
                  <tr><td colSpan="8" className="text-center py-4 text-muted">No revenues found</td></tr>
                  ...
                  <tr key={row.id}>
                    ...
                    <td>{row.category}</td>
                    <td><span className="badge bg-success-subtle text-success">{row.subCategory}</span></td>
            ...
            <div className="modal-dialog modal-dialog-centered modal-lg">
              <div className="modal-content border-0 shadow">
                <div className="modal-header">
                  <h4 className="modal-title fw-bold">Add Budget Revenue</h4>
        ...
        <>
          <div className="modal fade show" style={{ display: "block" }} tabIndex="-1">
            <div className="modal-dialog modal-dialog-centered modal-lg">
              <div className="modal-content border-0 shadow">
                <div className="modal-header">
                  <h4 className="modal-title fw-bold">Edit Budget Revenue</h4>
            ...
            <div className="modal-dialog modal-dialog-centered">
              <div className="modal-content border-0 shadow">
                ...
                <div className="modal-body">
                  <p>Are you sure you want to delete{deleteTarget?.name ? ` "${deleteTarget.name}"` : " this revenue"}?</p>
                </div>
                <div className="modal-footer">
