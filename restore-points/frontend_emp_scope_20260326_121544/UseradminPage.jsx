...
function UseradminPage() {
  ...
  useEffect(() => {
    const loadEmployeesData = async () => {
      try {
        // Load all employees - use the regular endpoint as fallback
  ...
  const availableEmployees = useMemo(() => {
    if (!rows || rows.length === 0) {
      return employees; // If no users loaded yet, show all employees
    }
    ...
    return employees.filter(
      (emp) =>
        emp.email && !userEmails.has(String(emp.email || "").toLowerCase().trim())
    );
  }, [employees, rows]);
